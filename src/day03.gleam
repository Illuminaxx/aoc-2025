...
fn find_max_pair_all(digits: List(String), len: Int, i: Int, current_max: Int) -> Int {
  case i >= len - 1 {
    True -> current_max
    False -> {
...
fn find_pairs_from_i(digits: List(String), i: Int, j: Int, current_max: Int) -> Int {
...
fn build_largest(remaining: List(String), needed: Int, acc: List(String)) -> List(String) {
  case needed {
    0 -> list.reverse(acc)
    _ -> {
          ...
          case int.parse(digit), int.parse(current_max) {
            Ok(d), Ok(m) -> {
              case d > m {
                ...
                False -> find_max_in_window(rest, index + 1, Some(current_max), max_index)
              }
            }
            _, _ -> find_max_in_window(rest, index + 1, Some(current_max), max_index)
          }
        }
      }
    }
